...
void SuperLIOReLoc::UpdateMap() {
  // In relocation mode, keep the global map fixed by default so each frame
  // is matched against the reference map instead of drifting into odometry mode.
  if (!g_update_map) {
    return;
  }
...
void SuperLIOReLoc::Output() {
  ...
  if(g_visual_map && data_wrapper_->has_cloud_world_subscribers()){
    static int count = -1;
    count++;
