...
void SuperLIO::ApplyZConstraint(){
  if(!g_z_constraint_enable) return;

  if(!z_constraint_ref_initialized_){
    z_constraint_ref_ = g_z_constraint_use_initial_height
      ? sys_init_pose_.t_(2)
      : static_cast<scalar>(g_z_constraint_reference);
    z_constraint_ref_initialized_ = true;
    LOG(INFO) << GREEN << " ---> [SuperLIO]: Z constraint enabled. reference z: "
              << z_constraint_ref_ << " m" << RESET;
  }

  kf_->ApplyVerticalConstraint(
    z_constraint_ref_,
    static_cast<scalar>(g_z_constraint_pos_std),
    static_cast<scalar>(g_z_constraint_vel_std),
    static_cast<scalar>(g_z_constraint_max_correction));
}
