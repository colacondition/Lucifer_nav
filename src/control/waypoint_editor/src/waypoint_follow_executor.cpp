...
class WaypointFollowExecutor : public rclcpp::Node
{
public:
  WaypointFollowExecutor()
  : Node("waypoint_follow_executor"),
    ...
    tf_listener_(std::make_shared<tf2_ros::TransformListener>(*tf_buffer_))
  {
    ...
    declare_parameter<std::string>("executor_status_topic", "/waypoint_editor/follow_status");
    ...
    declare_parameter<double>("status_timeout", 30.0);
    ...
    start_service_ = create_service<std_srvs::srv::Trigger>(
      ...
      std::bind(&WaypointFollowExecutor::handleStartFollow, this, std::placeholders::_1, std::placeholders::_2));

    RCLCPP_INFO(
      get_logger(), "Waypoint-follow executor ready: publish %s, watch %s",
      goal_topic_.c_str(), status_topic_.c_str());
    publishExecutorStatus("IDLE");
  }

  ~WaypointFollowExecutor() override
  {
...
private:
  ...
  void runFollow(std::vector<geometry_msgs::msg::PoseStamped> waypoints)
  {
    for (std::size_t i = 0; i < waypoints.size() && rclcpp::ok() && !stop_requested_; ++i) {
      ...
      publishApproachEnabled(is_final_goal);
      ...
      if (!waitUntilNearWaypoint(waypoints[i], i + 1, is_final_goal)) {
        publishApproachEnabled(true);
        worker_running_ = false;
        publishExecutorStatus(stop_requested_ ? "IDLE" : "ABORTED");
        if (!stop_requested_) {
          RCLCPP_WARN(get_logger(), "Waypoint-follow aborted");
        }
        return;
      }
    }
    ...
    if (!stop_requested_) {
      RCLCPP_INFO(get_logger(), "Waypoint-follow completed");
    }
  }

  void handleStartFollow(
    const std::shared_ptr<std_srvs::srv::Trigger::Request>,
    std::shared_ptr<std_srvs::srv::Trigger::Response> response)
  {
    if (worker_running_) {
      RCLCPP_INFO(get_logger(), "Restarting waypoint-follow on new start request");
      stop_requested_ = true;
      if (worker_.joinable()) {
        worker_.join();
      }
    ...
    if (waypoints.empty()) {
      response->success = false;
      response->message = "No waypoints to follow";
      return;
    }
    ...
    worker_ = std::thread(
      [this, waypoints = std::move(waypoints)]() mutable {runFollow(std::move(waypoints));});

    response->success = true;
    response->message = "Waypoint following started with " + std::to_string(waypoint_count) + " waypoints";
  }
...
int main(int argc, char ** argv)
{
  ...
  rclcpp::spin(std::make_shared<WaypointFollowExecutor>());
  rclcpp::shutdown();
  return 0;
}
