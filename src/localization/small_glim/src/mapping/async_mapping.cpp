...
AsyncMappingParams::AsyncMappingParams(const Config::Ptr& config) {
    ...
    output_dir = config->param<std::string>("mapping.output_dir");
    map_name = config->param<std::string>("mapping.map_name");
...
AsyncMapping::AsyncMapping(const Config::Ptr& config):
    params_(config),
    output_dir_(
        params_.output_dir.empty()
            ? (fs::path(params_.output_root) / ("mapping_" + make_timestamp_string())).string()
            : params_.output_dir
    ),
...
void AsyncMapping::save_final_map() {
    ...
    const std::string filename = params_.map_name.empty() ? "mapping.pcd" : params_.map_name;
    const fs::path filepath = fs::path(output_dir_) / filename;
    if (pcl::io::savePCDFileBinary(filepath.string(), *map_cloud_) != 0) {
        logger::warn("mapping", "failed to save {}", filepath.string());
    } else {
        logger::info("mapping", "saved final map {} (points={})", filepath.string(), map_cloud_->size());
    }
}
