...
| 配置项 | 填在哪 | 值（2026-09 实测标定） |
| :- | :- | :- |
| 驱动参数 | `config/reality/mid360_driver_real.yaml` | `host_ip: 192.168.1.50`（主机绑包 IP） |
| 雷达推流目标 | 出厂默认即正确，无需改（Viewer2/SDK 可改写，注意别被覆盖） | 推到 `192.168.1.50:56301`（点云）/ `:56401`（IMU） |
| 主机网卡 | 系统网络配置 | `enp7s0` 配静态 `192.168.1.50/24` |

当前实车雷达：**Mid-360s**，出厂 IP `192.168.1.101`（规则 `192.168.1.1XX`，XX = SN
尾两位）。可用发现协议查询：向 `192.168.1.255:56000` 单播/定向广播 0x0000 查询帧
（`src/bringup/tools/livox_discover.py`），应答含 dev_type（**35 = Mid-360s**）、SN、
lidar_ip。

注意 `host_ip` 填的是**工控机自己的 IP，不是雷达 IP**。三处任何一处不一致，表现都是
`ros2 topic hz /livox/lidar/pointcloud` 无数据。

两个坑：

- **Livox Viewer 2 和驱动共用 56301/56401 端口，不能同时开**。Viewer2 调完雷达
  （看数据、改配置）要先完全退出再起导航。
- 雷达出厂默认推流目标就是 `192.168.1.50`；若被改成别的网段（如旧文档的
  `192.168.10.x`），要么用 Viewer2 改回，要么把网卡和 `host_ip` 一起跟着改。
