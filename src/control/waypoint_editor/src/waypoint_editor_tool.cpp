...
void WaypointEditorTool::onInitialize()
{
    ...
    delete_last_service_ = nh_->create_service<std_srvs::srv::Trigger>(
        "delete_last_waypoint",
        std::bind(&WaypointEditorTool::handleDeleteLastWaypoint, this, _1, _2)
    );
    ...
    saved_waypoint_file_pub_ = nh_->create_publisher<std_msgs::msg::String>(
        "/waypoint_editor/saved_waypoint_file",
        rclcpp::QoS(rclcpp::KeepLast(1)).transient_local().reliable());
    current_waypoints_pub_ = nh_->create_publisher<nav_msgs::msg::Path>(
...
void WaypointEditorTool::publishCurrentWaypoints()
{
    ...
    current_waypoints_pub_->publish(path);
}

void WaypointEditorTool::recalculateWaypointDistances()
{
    total_wp_dist_ = 0.0;
    last_wp_dist_ = 0.0;
...
void WaypointEditorTool::handleSaveWaypoints(const std::shared_ptr<std_srvs::srv::Trigger::Request> /*req*/, std::shared_ptr<std_srvs::srv::Trigger::Response> res)
{
    ...
    ofs.close();
    std_msgs::msg::String saved_path_msg;
    saved_path_msg.data = path;
    saved_waypoint_file_pub_->publish(saved_path_msg);
    publishCurrentWaypoints();
    res->success = true;
...
void WaypointEditorTool::handleDeleteLastWaypoint(
    const std::shared_ptr<std_srvs::srv::Trigger::Request> /*req*/,
    std::shared_ptr<std_srvs::srv::Trigger::Response> res)
{
    if (waypoints_.empty()) {
        res->success = false;
        res->message = "No waypoint to delete";
        return;
    }

    const int deleted_id = static_cast<int>(waypoints_.size() - 1);
    waypoints_.pop_back();
    recalculateWaypointDistances();
    updateWaypointMarker();
    publishCurrentWaypoints();
    res->success = true;
    res->message = "Deleted waypoint " + std::to_string(deleted_id);
}
